...
namespace JesseRussell {
	namespace cmp {
		...
		int measureIntersection(const float& a1, const float& b1, const float& a2, const float& b2);
		...
		float& ref_closest(float& a, float& b, const float& from);
		float& ref_farthest(float& a, float& b, const float& from);
	...
	namespace vectors {
		...
		public:
			Cardinal() = default;
			Cardinal(Value value) { this->value = value; }
			Cardinal(uint8_t value) { this->value = static_cast<Value>(0b00001111 & value); }
		...
		public: // methods:
			...
			Cardinal AxisFilled() const {
				Cardinal result = NONE;
				if (isHorizontal()) result |= EAST_WEST;
				if (isVertical()) result |= NORTH_SOUTH;
				return result;
			}
