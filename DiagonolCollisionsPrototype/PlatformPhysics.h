...
namespace phy {
	...
	public: // constructors
		DynamicEntity(fvector2 position, fvector2 size) {
	...
	public: //methods
		bool intersectionHorizontal(const DynamicEntity& other, const float& x, const float& other_x) const {
			return rangeIntersection(x, x + size.x, other_x, other_x + other.size.x);
		}

		bool intersectionHorizontal(const DynamicEntity& other, const float& x) const {
			return rangeIntersection(x, x + size.x, other.position.x, other.getX2());
		}

		bool intersectionHorizontal(const DynamicEntity& other) const {
			return rangeIntersection(position.x, getX2(), other.position.x, other.getX2());
		}

		bool intersectionVertical(const DynamicEntity& other, const float& y, const float& other_y) const {
			return rangeIntersection(y, y + size.y, other_y, other_y + other.size.y);
		}

		bool intersectionVertical(const DynamicEntity& other, const float& y) const {
			return rangeIntersection(y, y + size.y, other.position.y, other.getY2());
		}

		bool intersectionVertical(const DynamicEntity& other) const {
			return rangeIntersection(position.y, getY2(), other.position.y, other.getY2());
		}

		bool intersection(const DynamicEntity& other, const fvector2& position, const fvector2& other_position) const {
			return vectorRangeIntersection(position, position + size, other_position, other_position + other.size);
		}

		bool intersection(const DynamicEntity& other, const fvector2& position) const {
			return vectorRangeIntersection(position, position + size, other.position, other.position + other.size);
		}

		bool intersection(const DynamicEntity& other) const {
			return vectorRangeIntersection(position, position + size, other.position, other.position + other.size);
		}
	...
	public: // Methods:
		...
		bool collisionStaticCheck(const DynamicEntity& other, float timeScale) {
			fvector2 trash;
			CardinalDirection alsoTrash;
			return internal_collisionStaticCheck<false> (other, timeScale, trash, alsoTrash);
		}

		bool collisionStaticCheck(const DynamicEntity& other, float timeScale, fvector2& collisionSpot, CardinalDirection& collisionSide) {
	...
	private: // Methods
		...
		bool internal_collisionStaticCheck(const DynamicEntity& other, float timeScale, fvector2& out_collisionSpot, CardinalDirection& out_collisionSide) const {
