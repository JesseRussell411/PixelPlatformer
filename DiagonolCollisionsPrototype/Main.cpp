...
public:
	...
	phy::DynamicEntity steve = phy::DynamicEntity(fvector2(100, 100), fvector2(30, 30));
	phy::DynamicEntity frank = phy::DynamicEntity(fvector2(69, 100), fvector2(5, 5));

	void DrawEntity(const phy::DynamicEntity& entity, olc::Pixel color = olc::WHITE) {
		DrawRect(entity.getX1(), entity.getY1(), entity.getWidth(), entity.getHeight(), color);
	...
	bool OnUserUpdate(float fElapsedTime) override
	{
		...
		if (steve_col) {
			...
			case phy::NORTH:
				highest_y_col = highest_y_defined ? cmp::min(steve_collisionSpot.y, highest_y_col) : steve_collisionSpot.y;
				highest_y_defined = true;
				break;
			case phy::EAST:
				lowest_x_col = lowest_x_defined ? cmp::min(steve_collisionSpot.x, lowest_x_col) : steve_collisionSpot.x;
				lowest_x_defined = true;
				break;
			case phy::SOUTH:
				lowest_y_col = lowest_y_defined ? cmp::max(steve_collisionSpot.y, lowest_y_col) : steve_collisionSpot.y;
				lowest_y_defined = true;
				break;
			case phy::WEST:
				highest_x_col = highest_x_defined ? cmp::min(steve_collisionSpot.x, highest_x_col) : steve_collisionSpot.x;
				highest_x_defined = true;
				break;
			}
		}
